...
function Profile() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Split skills by comma and send as JSON or array
        // We'll send as string to backend which then splits, OR we do it here. 
        // Backend expects `skills` field. If we append multiple `skills` keys, multer handles it?
        // Actually backend logic `user.skills = req.body.skills.split(...)` handles the string.
        ...
        <div className="max-w-4xl mx-auto py-10 fade-in px-4">
            <Card className="p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Input label="Email Address" name="email" value={formData.email} disabled className="bg-slate-50 cursor-not-allowed text-slate-500" />
                        <Input label="Location" name="location" value={formData.location} onChange={handleChange} placeholder="e.g. New York, USA" />
                    </div>

                    <Input label="Bio" type="textarea" name="bio" value={formData.bio} onChange={handleChange} placeholder="Tell us about yourself..." />
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-4">
                            <label className="flex items-center justify-center px-4 py-2 border border-slate-300 rounded-xl shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-slate-50 cursor-pointer">
                            ...
                            {user?.resume && (
                                <a href={user.resume} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline">
                                    View Current Resume
                                </a>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            {experience.map((exp, index) => (
                                <div key={index} className="p-4 bg-slate-50 rounded-xl border border-slate-200 relative group">
                                    ...
                                    <Input type="textarea" placeholder="Description" value={exp.description} onChange={(e) => handleExperienceChange(index, "description", e.target.value)} className="min-h-[60px]" />
                                </div>
                            ))}
                            {experience.length === 0 && (
                    ...
                    <div className="pt-4 border-t border-slate-100 mt-6">
                        <Input label="New Password (Leave blank to keep current)" type="password" name="password" value={formData.password} onChange={handleChange} placeholder="••••••••" />
                    </div>

                    <div className="pt-4 mt-6">
                        <Button type="submit" variant="primary" isLoading={loading} className="w-full shadow-lg shadow-primary/20">
